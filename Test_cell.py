            ...
            if inputs_checks and all_inputs:
                ...
                if  len(Fun.outputs) == 1:
                    if np.isfinite(algae_data[Fun.outputs[0]][i]):
                        ...
                        if (abs(res_algae) >= 60 or abs(rel_res_algae) > 1) and (Fun.outputs == 'Biocrude wt%' or Fun.outputs == ['Biocrude wt%']):
                    ...
                    for o in Fun.outputs:
                        if np.isfinite(algae_data[o][i]) and  np.isfinite(result[count]):
                            ...
                            if (abs(res_algae[count]) >= 60 or abs(rel_res_algae[count]) > 1) and (o == 'Biocrude wt%' or o ==['biocrude wt%']):
                                name = pd.Series(['Algae'])
                                name.to_excel(outliers,startcol=col_1  ,startrow =0,index=False,header=False, sheet_name =  Fun.fun.__name__)
                                name = pd.Series([Fun.fun.__name__])
                                name.to_excel(outliers,startcol=col_1  ,startrow =1,index=False,header=False, sheet_name =  Fun.fun.__name__)
            ...
            if inputs_checks == True and check_require_one == True:
                    ...
                    if  len(Fun.outputs) == 1:
                        if np.isfinite(biomass_data[Fun.outputs[0]][i]) and  np.isfinite(result):
                            ...
                            if (abs(res_biomass) >= 60 or abs(rel_res_biomass) > 1) and (Fun.outputs == 'Biocrude wt%' or Fun.outputs == ['Biocrude wt%']):
                        ...
                        for o in Fun.outputs:
                            if np.isfinite(biomass_data[o][i]) and  np.isfinite(result[count]):
                                ...
                                if (abs(res_biomass[count]) >= 60 or abs(rel_res_biomass[count]) > 1) and (o == 'Biocrude wt%' or o == ['Biocrude wt%']):
                                    name = pd.Series(['Biomass'])
                                    name.to_excel(outliers,startcol=col_1  ,startrow =0,index=False,header=False, sheet_name =  Fun.fun.__name__)
                                    name = pd.Series([Fun.fun.__name__])
                                    name.to_excel(outliers,startcol=col_1  ,startrow =1,index=False,header=False, sheet_name =  Fun.fun.__name__)
                        ...
                        if not opt == []:
                            ...
                            for me in opt:
                                if me == "Cellulose wt%" or me == "Hemicellulose wt%" or me == "Starch wt%" :
                                    ...
                                    if me == "Cellulose wt%":
                                        if "Hemicellulose wt%" in opt:
                                            poss_val.insert(0,np.nansum([biomass_data["Carbs wt%"][i],-biomass_data["Hemicellulose wt%"][i]]))
                                        elif "Starch wt%" in opt:
                                            poss_val.insert(0,np.nansum([biomass_data["Carbs wt%"][i],-biomass_data["Starch wt%"][i]]))
                                        else:
                                            poss_val.insert(0,biomass_data["Carbs wt%"][i])
                                    elif me == "Hemicellulose wt%" or me == "Starch wt%":
...
def Yang2019(const):
    ...
    if Time <= 20 and Time >= 5 and Temp <= 320 and Temp >= 270 and Loading >= 5  and Loading <= 20:
...
out = pd.ExcelWriter('Data_out_cell.xlsx')
