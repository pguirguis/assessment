...
def analysis(Res,rel_Res,N_ks,num_outs):
    if Res == [] :
        ...
        if num_col == 1:
            ...
            mean_Res = np.append(mean_Res,np.nansum(Res)/n_point[num_col-1])
...
def Model_analysis(Fun,if_mat):
    ...
    for i in range(len(algae_data)):
        ...
        if  inputs_checks ==  True and len(require_one) == len(Fun.require_one) :
            ...
            if not opt == []:
                for me in opt:
                    in_pos = np.ones(len(Fun.optional))
                    if inputs_checks == True:
                        if me == 'Cellulose wt%' and  not 'Carbs wt%' in opt:
                            opt_poss.insert(0,'Carbs wt%')
                        elif not me in algae_check:
                            opt_poss.insert(0,'0')
                            in_pos[Fun.optional.index(me)] = 0
                        else:
                ...
                intended = all(x in  algae_check  for x in opt)
            else:
                intended = True
                all_inputs = True 
            sum_fin = np.isfinite(algae_data['Sum wt%'][i])
                    
            if inputs_checks and all_inputs and sum_fin:
                ...
                if  len(Fun.outputs) == 1:
                    if np.isfinite(algae_data[Fun.outputs[0]][i]):
                        ...
                        if  (Fun.outputs == 'Biocrude wt%' or Fun.outputs == ['Biocrude wt%']):
                    ...
                    for o in Fun.outputs:
                        if np.isfinite(algae_data[o][i]) and  np.isfinite(result[count]):
                            ...
                            if (o == 'Biocrude wt%' or o ==['biocrude wt%']):
                                name = pd.Series(['Algae'])
                                name.to_excel(outliers,startcol=col_1  ,startrow =0,index=False,header=False, sheet_name =  Fun.fun.__name__)
                                name = pd.Series([Fun.fun.__name__])
                                name.to_excel(outliers,startcol=col_1  ,startrow =1,index=False,header=False, sheet_name =  Fun.fun.__name__)
                ...
                if len(in_pos)>1:
                    Fun.add_res_biomass(res_algae,[list(in_pos)])
                    Fun.add_rel_res_biomass(rel_res_algae,[list(in_pos)])
                if intended == True:
                ...
                for k in Fun.require_one:
                    ...
                    for q in k:
                        if np.isfinite(biomass_data[q][i]) == True and check_require_one == False:
                            require_one.insert(0, q)
                            check_require_one = True
            sum_fin = np.isfinite(biomass_data['Sum wt%'][i])
            if inputs_checks == True and check_require_one == True and sum_fin:
...
mod_Valdez2013 = def_mod('Valdez2013',22,['Temperature (C)'],[['Total time (min)','Holding Time (min)']],['b'],['Solids wt%','Biocrude wt%','Light Biocrude wt%','Heavy Biocrude wt%','Aquous wt%','Gas + Loss','Aq + Gas wt%'],['Carbs wt%','Protein wt%','Lipids wt%','Lignin wt%'],[],False)
...
mod_Hietala2016 = def_mod('Hietala2016',14,['Temperature (C)'],[['Total time (min)','Holding Time (min)']],['b'],['Solids wt%','Biocrude wt%','Aquous wt%','Gas wt%', 'Loss' ,'Gas + Loss','Aq + Gas wt%'],['Carbs wt%','Protein wt%','Lipids wt%','Lignin wt%'],[],False)
...
mod_Qian2020 = def_mod('Qian2020',10,['Temperature (C)'],[['Total time (min)','Holding Time (min)']],['b'],['Solids wt%','Biocrude wt%','Aquous wt%','Gas + Loss','Aq + Gas wt%'],['Carbs wt%','Protein wt%','Lipids wt%','Lignin wt%'],[],False)
...
mod_Saral2022 = def_mod('Saral2022',6*2,['Temperature (C)'],[['Total time (min)','Holding Time (min)']],['b'],['Solids wt%','Biocrude wt%','Aquous wt%','Gas + Loss','Aq + Gas wt%'],['Carbs wt%','Protein wt%','Lipids wt%','Lignin wt%'],[],False)
...
def Teri2014_3(const):
    ...
    SI = np.log(Time*np.exp(-210000/8.314*(1/(Temp+273.15)-1/(700))))


    xL,xC,xP = Lip/100,Carb/100,Prot/100

    if SI < np.log(40*np.exp(-210000/8.314*(1/(325+273.15)-1/(700)))) :
...
mod_Denial2017 = def_mod('Denial2017',8,['Temperature (C)'],[['Total time (min)','Holding Time (min)']],[],['Solids wt%','Biocrude wt%','Aquous wt%','Gas + Loss','Aq + Gas wt%'],['Carbs wt%','Protein wt%','Lipids wt%','Lignin wt%'],[],False)
...
def Shakya2017(const):
    ...
    if SI < np.log(30*np.exp(-210000/8.314*(1/(300+273.15)-1/(700)))) :
...
def Wagner2016(const):
    ...
    SI = np.log(t*np.exp(-210000/8.314*(1/(Temp+273.15)-1/(700))))


    if SI < np.log(30*np.exp(-210000/8.314*(1/(310+273.15)-1/(700)))) :
        Y_Crude = 0.96*L + 0.024*C + 0.161*P
    elif SI > np.log(30*np.exp(-210000/8.314*(1/(310+273.15)-1/(700)))) and SI < np.log(30*np.exp(-210000/8.314*(1/(330+273.15)-1/(700)))):  
        Y_Crude = 1.001*L + 0.033*C + 0.244*P
    elif SI > np.log(30*np.exp(-210000/8.314*(1/(330+273.15)-1/(700)))) and SI < np.log(30*np.exp(-210000/8.314*(1/(350+273.15)-1/(700)))):  
...
def Subramanya2021(const):
    ...
    if SI < np.log(30*np.exp(-210000/8.314*(1/(325+273.15)-1/(700)))):

        if Prot == 0 or Cell == 0:
            PC = 0
        else:
            PC = 0.0139*Prot*Cell/(Prot + Cell)**2
    ...
    elif SI > np.log(30*np.exp(-210000/8.314*(1/(387.5+273.15)-1/(700)))) :
...
out = pd.ExcelWriter('Data_out_components.xlsx')
def pri_vals(mod,test_in,test_out,biomass_in,exclu_biomass,col_n):
    all_carbs = ['Carbs wt%','Cellulose wt%','Hemicellulose wt%','Starch wt%']
    list_num = 0
    if test_in == "biomass":
        ...
        for mass in biomass_in:
            if isinstance(mass, list) :
                ...
                for magic in mass:
                    if magic == 'Carbs wt%':
                        check = np.append(check,any([item in mod.optional for item in all_carbs]))
                        
                    else:
                        check = np.append(check,magic in mod.optional)
                if all(check):
                    p = np.zeros(len(mod.optional))
                    for v in mass:
                        if v == 'Carbs wt%':
                            for me in all_carbs:
                                if me in  mod.optional:
                                    ind = mod.optional.index(me)
                                    p[ind] = 1
                        else:
                            ind = mod.optional.index(v)
                            p[ind] = 1
                    p = list(p)
                    she_name = test_in + '_' + 'list'+ str(list_num) + '_res'
                    she2_name =  test_in + '_' + 'List' + str(list_num) + '_analysis'
                ...
                check = []
                if mass =='Carbs wt%':
                    check = any([item in mod.optional for item in all_carbs])
                else:
                    check = mass in mod.optional
                if check == True:
                    p = np.zeros(len(mod.optional))
                    if mass == 'Carbs wt%':
                        for me in ['Carbs wt%','Cellulose wt%','Hemicellulose wt%','Starch wt%']:
                            if me in mod.optional:
                                ind = mod.optional.index(me)
                                p[ind] = 1
                    else:
...
Test_in = [[],'intended','algae','biomass']
Test_out = 'Biocrude wt%'
biomass_in = ['Protein wt%','Carbs wt%','Lipids wt%','Lignin wt%',['Carbs wt%','Protein wt%'],['Carbs wt%','Lignin wt%'],['Carbs wt%','Protein wt%','Lipids wt%'],['Carbs wt%','Protein wt%','Lipids wt%','Lignin wt%']]
exclu_biomass = []

col = 0

for i in [mod_Biller2011,mod_Teri2014_1,mod_Teri2014_2,mod_Leow2015,mod_Wagner2016,mod_Li2017,mod_Shakya2017,mod_Teri2014_3,mod_Teri2014_4,mod_Denial2017,mod_Yang2018,mod_Lu2018,mod_Sheng2018,mod_Subramanya2021,mod_Hietala2017,mod_Yang2019,mod_Aierzhati2019,mod_Yan2022]:
    for l in Test_in:
...
# for i in [mod_Luo2016,mod_Sheehan2016,mod_Chacon_Parra2021,mod_Valdez2013,mod_Saral2022,mod_Hietala2016,mod_Qian2020,mod_Valdez2014,mod_Vo2016,mod_Vo2017,mod_Sheehan2017_1,mod_Palomino2019,mod_Obeid2022_algae,mod_Sheehan2017_2,mod_Obeid2019,mod_Obeid2020_poly,mod_Obeid2020_mono,mod_Obeid2022_Sludge,mod_Obeid2022_Pinwood]:
    for l in Test_in:
        pri_vals(i,l,Test_out,biomass_in,exclu_biomass,col)
    col = col +1 
